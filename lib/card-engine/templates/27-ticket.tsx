...
const template: CardTemplate = {
  ...
  render({ data, tokens, thumbnail, dimensions, strip }) {
    ...
    if (strip) return renderStrip(data, tokens, {
      decoratives: (
        <>
          <div style={{ position: "absolute", top: 7, left: 0, right: 0, display: "flex", gap: 10, justifyContent: "center", zIndex: 2 }}>
            {Array.from({ length: 28 }).map((_, i) => (
              <div key={i} style={{ width: 5, height: 5, borderRadius: "50%", background: tokens.surface }}/>
            ))}
          </div>
          <div style={{ position: "absolute", bottom: 7, left: 0, right: 0, display: "flex", gap: 10, justifyContent: "center", zIndex: 2 }}>
            {Array.from({ length: 28 }).map((_, i) => (
              <div key={i} style={{ width: 5, height: 5, borderRadius: "50%", background: tokens.surface }}/>
            ))}
          </div>

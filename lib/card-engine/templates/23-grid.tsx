...
const template: CardTemplate = {
  ...
  render({ data, tokens, thumbnail, dimensions, strip }) {
    ...
    if (strip) return renderStrip(data, tokens, {
      decoratives: (
        <div style={{ position: "absolute", inset: 0, backgroundImage: `linear-gradient(${tokens.accent}14 1px, transparent 1px), linear-gradient(90deg, ${tokens.accent}14 1px, transparent 1px)`, backgroundSize: "24px 24px" }}/>
      ),
    });

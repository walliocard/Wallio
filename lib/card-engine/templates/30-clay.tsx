...
const template: CardTemplate = {
  ...
  render({ data, tokens, thumbnail, dimensions, strip }) {
    ...
    if (strip) return renderStrip(data, tokens, {
      decoratives: (
        <>
          <div style={{ position: "absolute", left: "82%", top: "14%", width: 76, height: 76, borderRadius: "50%", border: `1.5px solid ${tokens.accent}`, opacity: 0.12, transform: "translate(-50%,-50%)" }}/>
          <div style={{ position: "absolute", left: "14%", top: "76%", width: 52, height: 52, borderRadius: "50%", border: `1.5px solid ${tokens.accent}`, opacity: 0.12, transform: "translate(-50%,-50%)" }}/>
        </>
      ),
    });
    ...
    return (
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: thumbnail ? "6% 7% 3%" : `${7*fmtV}% 8% ${4*fmtV}%`, position: "relative", zIndex: 1 }}>
          ...
          <div style={{ background: `${tokens.accent}20`, backdropFilter: "blur(8px)", borderRadius: 20, padding: thumbnail ? "1px 5px" : "2px 8px", border: `1px solid ${tokens.accent}35`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>

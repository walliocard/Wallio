...
export default function AppleWalletCard({
  ...
  return (
    ...
    >
      ...
      {/* ── Champs — une seule ligne (rendu réel Apple Wallet) ── */}
      {(() => {
        ...
        return (
          <div style={{ display: "flex", padding: "12px 16px 14px", gap: 6 }}>
            {allFields.map((f, i) => (
              <div key={i} style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 9, color: labelClr, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>
                  {f.label || "INFO"}
                </div>
                <div style={{ fontSize: i < 2 ? 14 : 12, fontWeight: 500, color: fg, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {f.value}
                </div>
              </div>
            ))}
